...
const IGNORED_DIRS = new Set(['node_modules', '.git', 'dist', 'build', '.next', '.nuxt', 'out', 'coverage', '.svelte-kit', '.astro', '.vercel', '.output'])
...
// ---- main --------------------------------------------------------------------
const { compileToSource, subsetConfig, mergeConfigs } = await import('../dist/compiler.js')
...
if (!opts.full) {
    if (opts.tokens) {
        for (const t of readFileSync(resolve(opts.cwd, opts.tokens), 'utf8').split(/\s+/)) {
            if (t) tokens.add(t)
        }
    } else {
        ...
        if (files.length === 0) fail('no files matched the content globs; pass --content or use --full')
    ...
    if (opts.safelist) {
        for (const t of readFileSync(resolve(opts.cwd, opts.safelist), 'utf8').split(/\s+/)) {
            if (t) tokens.add(t)
        }
    }
}
...
if (!opts.quiet) {
    ...
    const parts = []
    if (!opts.full) {
        parts.push(`${scannedFiles ? `scanned ${scannedFiles} files, ` : ''}${tokens.size} candidate tokens`)
        parts.push(`${usedGroups}/${totalGroups} class groups kept`)
    } else {
        parts.push('all class groups kept (--full)')
    }
    console.log(`cn build: ${parts.join(', ')}`)
    console.log(`  → ${opts.out} (${(source.length / 1024).toFixed(1)} KB source, ${(gz / 1024).toFixed(1)} KB gzip)`)
}
