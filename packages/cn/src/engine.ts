...
const spanHash = (str: string, s: number, e: number): number => {
    ...
    if (len > 3) {
        ...
        const m = len >> 1
        h = Math.imul(
            h ^ (str.charCodeAt(s + 1) << 8) ^ (str.charCodeAt(s + 2) << 16) ^ str.charCodeAt(s + q),
            0x85ebca6b,
        )
        h = Math.imul(
            h ^ (str.charCodeAt(s + m) << 8) ^ (str.charCodeAt(s + m + q) << 16) ^ str.charCodeAt(e - 3),
            0xc2b2ae35,
        )
...
export const createEngine = (
    ...
    const {
        GROUP_COUNT,
        edgeStart, labelStart, labelText, edgeTarget, nodeGroup, nodeVlist,
        vlistPat, vlistOps, vlistRef, vlistGroup,
        litAnchor, litGroup, litPool, poolOffsets, poolText,
        adjGid, adjStart, adjTgt, patGid, patTgt,
        postfixLookupGroups, customValidatorNames, orderSensitiveModifiers,
    } = T
    ...
    for (let l = 0; l < vlistRef.length; l++) {
        vgStart[l + 1] = vgStart[l] + vlistPat[vlistRef[l] + 1] - vlistPat[vlistRef[l]]
    }
    ...
    for (let i = 0; i < litAnchor.length; i++) {
        ...
        let idx = ((fnv(poolText, off, off + poolOffsets[litPool[i] * 2 + 1]) ^
            Math.imul(litAnchor[i], 0x9e3779b1)) | 0) & (LIT_SIZE - 1)
        while (litTable[idx] !== -1) idx = (idx + 1) & (LIT_SIZE - 1)
        litTable[idx] = i
    }
    const litProbe = (anchor: number, input: string, s: number, e: number): number => {
        let idx = ((fnv(input, s, e) ^ Math.imul(anchor, 0x9e3779b1)) | 0) & (LIT_SIZE - 1)
        const len = e - s
        for (;;) {
            ...
            if (litAnchor[entry] === anchor && poolOffsets[litPool[entry] * 2 + 1] === len) {
                ...
                let ok = true
                for (let k = 0; k < len; k++) {
                    if (poolText.charCodeAt(off + k) !== input.charCodeAt(s + k)) { ok = false; break }
                }
    ...
    const spanEq = (input: string, s: number, e: number, str: string): boolean => {
    ...
    const spanIsNamedContainerQuery = (input: string, s: number, e: number): boolean => {
        ...
        return (c11 === 115 && e - s >= 17 && spanEq(input, s + 10, s + 16, '-size/')) ||
    ...
    const runValidator = (op: number, input: string, s: number, e: number): boolean => {
        if (op >= 10) {
    ...
    interface InternEntry { k: string; imp: number; id: number }
    const internSpan = (map: Map<number, InternEntry[]>, input: string, s: number, e: number, imp: number, make: (raw: string) => number): number => {
        ...
        if (mods.length > 1) {
            ...
            for (const mod of mods) {
                if (mod.charCodeAt(0) === 91 || orderSensitive.has(mod)) {
                    if (segment.length) { result.push(...segment.sort()); segment = [] }
                    result.push(mod)
    ...
    let cap = 256
    let tokI32 = [new Int32Array(cap), new Int32Array(cap), new Int32Array(cap), new Int32Array(cap)]
    let [tokStart, tokEnd, tokGid, tokCtx] = tokI32
    let tokFlags = new Uint8Array(cap)
    let keep = new Uint8Array(cap)
    const growTokens = () => {
        cap *= 2
        tokI32 = tokI32.map((a) => { const n = new Int32Array(cap); n.set(a); return n })
        ;[tokStart, tokEnd, tokGid, tokCtx] = tokI32
        const nf = new Uint8Array(cap); nf.set(tokFlags); tokFlags = nf
    ...
    const resolveAt = (input: string, bs: number, endPos: number, nodeAt: number, ckptAt: number): number => {
        // arbitrary property: '[prop:...]' → dynamic per-property group
        if (endPos - bs >= 2 && input.charCodeAt(bs) === 91 &&
            input.charCodeAt(endPos - 1) === 93) {
            let colon = -1
            for (let p = bs + 1; p < endPos - 1; p++) {
                if (input.charCodeAt(p) === 58) { colon = p; break }
            }
    ...
    const mergeClassList = (input: string): string => {
        ...
        while (i < n) {
            ...
            while (i < n) {
                ...
                if (c <= 32) {
                    if (c === 32) break
                    if (c >= 9 && c <= 13) { sawNonSpaceWS = true; break }
            ...
            const way0 = (h & (TOKEN_TABLE - 1)) & ~1
            {
                ...
                if (hitAt >= 0) {
                    ...
                    let ok = true
                    for (let k = 0; k < len; k++) {
                        if (s.charCodeAt(k) !== input.charCodeAt(ts + k)) { ok = false; break }
                    }
                    if (ok) {
            ...
            for (let p = pts; p < te; p++) {
                ...
                if (depthB === 0 && depthP === 0) {
                    if (pc === 58) { lastColon = p; continue }
                    if (pc === 47) { lastSlash = p; continue }
                }
                if (pc === 91) depthB++
            ...
            for (let p = feedStart; p < be; p++) {
                ...
                if (node !== DEAD) {
                    ...
                    if (arrived >= 0 && (nodeVlist[arrived] >= 0 || nodeHasLit[arrived] === 1) &&
                        p + 1 < be && input.charCodeAt(p + 1) === 45) {
                        if (ckptTop === ckptCap) {
                            ckptCap *= 2
                            const nv = new Int32Array(ckptCap); nv.set(ckptNode); ckptNode = nv
                            const nt = new Int32Array(ckptCap); nt.set(ckptTail); ckptTail = nt
                        }
            ...
            if (gid === EXTERNAL) {
                ...
                ctxId = pts >= lastColon
                    ? (important ? 1 : 0)
                    : internSpan(ctxByHash, input, pts, lastColon, important ? 1 : 0,
        ...
        if (tokenCount * 32 > CLAIM_TABLE) {
            while (tokenCount * 32 > CLAIM_TABLE) { CLAIM_TABLE <<= 1; claimShift-- }
        ...
        for (let t = tokenCount - 1; t >= 0; t--) {
            const gid = tokGid[t]
            if (gid === EXTERNAL) { keep[t] = 1; continue }
            ...
            if (gid < GROUP_COUNT) {
                ...
                const r = adjRow[gid]
                if (r >= 0) {
                    for (let k = adjStart[r]; k < adjStart[r + 1]; k++) claimTest(ctxId, adjTgt[k])
                }
                if (tokFlags[t] & 1) {
        ...
        let t = 0
        while (t < tokenCount) {
            if (!keep[t]) { t++; continue }
            const runStart = tokStart[t]
            let runEnd = tokEnd[t]
            let u = t + 1
            while (
                u < tokenCount && keep[u] &&
                tokStart[u] === runEnd + 1 && input.charCodeAt(runEnd) === 32
            ) {
                runEnd = tokEnd[u]
                u++
            }
    ...
    const mergeString = cacheSize > 0
        ? (input: string): string => {
              ...
              const seen = door[slot] === (h ^ doorEpoch) || door[slot ^ DOOR_SIZE] === (h ^ (doorEpoch - 1))
              if (seen) {
                  r = prevCache[input]
                  if (r !== undefined) {
...
const resolveValue = (v: ClassValue, clsxMode: boolean): string => {
    ...
    let out = ''
    if (typeof (v as { length?: unknown }).length === 'number' && Array.isArray(v)) {
        for (let i = 0; i < v.length; i++) {
            const item = v[i]
            if (!item) continue
            const r = typeof item === 'string' ? item : resolveValue(item, clsxMode)
            if (r) { if (out) out += ' '; out += r }
        }
        return out
    }
    if (clsxMode) {
        ...
        if (typeof v === 'object') {
            for (const k in v) if ((v as Record<string, unknown>)[k]) { if (out) out += ' '; out += k }
        }
    }
    return out
}
...
const joinArgs = (args: IArguments, clsxMode: boolean): string => {
    ...
    for (let i = 0; i < args.length; i++) {
        const a = args[i]
        if (!a) continue
        const r = typeof a === 'string' ? a : resolveValue(a as ClassValue, clsxMode)
        if (r) { if (s) s += ' '; s += r }
    }
    return s
}
...
export const wrapClsx = (mergeString: (input: string) => string): CnFunction => {
    ...
    // resolve path below.
    const match3 = (e: ArgEntry, v0: ClassValue, v1: ClassValue, v2: ClassValue): boolean => {
        let k = 0
        if (v0) { if (v0 !== e.a0) return false; k = 1 }
        if (v1) { if (v1 !== (k === 0 ? e.a0 : e.a1)) return false; k++ }
        if (v2) { if (v2 !== (k === 0 ? e.a0 : k === 1 ? e.a1 : e.a2)) return false; k++ }
        return k === e.t
    }
...
export const createCn = (
    ...
    validatorImpls?: ValidatorImpls,
    options?: EngineOptions,
): CnFunction => wrapClsx(createEngine(tables, validatorImpls, options).mergeString)
