...
const measure = async (label, entrySource) => {
    ...
    const r = await build({
        ...
        write: false, target: 'es2022', absWorkingDir: pkgRoot,
...
const rows = [
    ...
    await measure('clsx+tailwind-merge', `import { clsx } from 'clsx'; import { twMerge } from 'tailwind-merge'; export const cn = (...a) => twMerge(clsx(a))`),
    await measure('cnfast', `export { cn } from 'cnfast'`),
]
for (const r of rows) console.log(r.label.padEnd(22), String(r.min).padStart(7), 'min', String(r.gz).padStart(7), 'min+gz')
...
if (ours.min >= cnfast.min) {
    console.error(`SIZE GATE FAIL (parse): cn ${ours.min} >= cnfast ${cnfast.min}`)
    fail = true
}
if (ours.gz > cnfast.gz * 1.08) {
    console.error(`SIZE GATE FAIL (gzip band): cn ${ours.gz} > cnfast ${cnfast.gz} * 1.08`)
    fail = true
}
if (ours.gz > 10500) {
...
console.log(`size gate ok: parse ${ours.min} < ${cnfast.min}; gzip ${ours.gz} (cnfast ${cnfast.gz}, band ${Math.round(cnfast.gz * 1.08)}); budget 10500`)
