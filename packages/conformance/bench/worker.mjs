...
const shortTypical = [
    ...
    'text-muted-foreground text-sm', 'font-semibold leading-none tracking-tight',
    'absolute right-4 top-4 rounded-sm opacity-70 transition-opacity',
    'fixed inset-0 z-50 bg-black/80', 'grid gap-4 py-4', 'flex flex-col space-y-1.5',
    'p-6 pt-0', 'text-lg font-semibold', 'mr-2 h-4 w-4', 'h-px w-full bg-border',
    'px-2 py-1 text-xs', 'w-full p-4 md:p-6 lg:px-8',
]

const variantPool = ['hover:', 'focus:', 'md:', 'lg:', 'dark:', 'group-hover:', 'data-[state=open]:', 'aria-checked:']
const basePool = ['p-1', 'px-2', 'm-3', 'w-4', 'text-lg', 'bg-red-500', 'flex', 'rounded-md', 'border-2', 'gap-2', 'shadow-lg', 'opacity-50', 'z-10', 'top-1', 'inset-x-2']
...
const arbHeavy = Array.from({ length: 50000 }, (_, i) =>
...
const vocab = ['flex', 'inline-flex', 'grid', 'block', 'items-center', 'items-start',
    'justify-between', 'justify-center', 'gap-1', 'gap-2', 'gap-4', 'p-1', 'p-2', 'p-4',
    'px-2', 'px-3', 'px-4', 'py-1', 'py-2', 'rounded', 'rounded-md', 'rounded-lg',
    'border', 'border-input', 'bg-background', 'bg-primary', 'bg-accent', 'text-sm',
    'text-xs', 'text-lg', 'font-medium', 'font-semibold', 'text-muted-foreground',
    'text-primary-foreground', 'shadow-sm', 'shadow', 'transition-colors',
    'hover:bg-accent', 'hover:text-white', 'focus:ring-2', 'md:flex-row', 'md:p-6',
    'dark:bg-gray-900', 'w-full', 'h-10', 'h-9', 'opacity-50', 'underline', 'truncate',
...
const repeated = ['inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm hover:bg-primary/90']
...
const loadImpl = async () => {
    if (implName === 'tailwind-merge') return (await import('tailwind-merge')).twMerge
    if (implName === 'pair') {
...
console.log(JSON.stringify({ impl: implName, workload: workloadName, nsPerOp: best, sink: sink % 10 }))
