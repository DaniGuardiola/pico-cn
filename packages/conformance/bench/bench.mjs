...
for (const [key, label] of workloads) {
    ...
    for (const impl of impls) {
        const out = execFileSync(process.execPath, [worker, impl, key], { encoding: 'utf8' })
        rows.push(JSON.parse(out.trim().split('\n').pop()))
    }
    const bestCached = Math.min(...rows.filter((r) => r.impl !== 'cn-nocache').map((r) => r.nsPerOp))
    for (const r of rows) {
...
const componentWorker = fileURLToPath(new URL('./component-worker.mjs', import.meta.url))
...
for (const [mode, label] of componentModes) {
    ...
    for (const impl of ['pair', 'cnfast', 'cn']) {
        const out = execFileSync(process.execPath, [componentWorker, impl, mode], { encoding: 'utf8' })
        rows.push(JSON.parse(out.trim().split('\n').pop()))
    }
