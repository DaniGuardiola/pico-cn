...
if (!skipBench) {
    ...
    const rows = [
        { key: 'component', label: 'the call your components make most', small: 'cn(base, variant, condition &amp;&amp; extra)', data: component },
    ...
    const initOf = (imports, call) =>
        Number(
            execFileSync(
                process.execPath,
                ['--input-type=module', '-e', `${imports}; const t0 = performance.now(); ${call}; console.log(performance.now() - t0)`],
                { encoding: 'utf8', cwd: conformance },
            ).trim(),
        ) * 1e6
    process.stderr.write('init … ')
    const init = {
        pair: initOf(`const { clsx } = await import('clsx'); const { twMerge } = await import('tailwind-merge')`, `twMerge(clsx('p-1 p-2'))`),
        cnfast: initOf(`const { cn } = await import('cnfast')`, `cn('p-1 p-2')`),
        cn: initOf(`const { cn } = await import('cn')`, `cn('p-1 p-2')`),
    }
    process.stderr.write('\n')
    rows.push({ key: 'init', label: 'very first call', small: 'page load', data: init })

    for (const row of rows) {
        const d = row.data
        const min = Math.min(d.pair, d.cnfast, d.cn)
        const cell = (v) => (v <= min * 1.07 ? `<td class="win">${fmtNs(v)}</td>` : `<td>${fmtNs(v)}</td>`)
        referenceRows.push(
...
if (corporaDir && !skipBench) {
    ...
    const gm = (k) => Math.exp(rows.reduce((s, r) => s + Math.log(r.cn / r[k]), 0) / rows.length)
    const fmtOps = (v) => (v >= 100 ? Math.round(v).toLocaleString('en-US') : v.toFixed(1))
    const body = rows
        .map(
            (r) =>
...
const sizes = {
    pair: await measureSize(`import { clsx } from 'clsx'; import { twMerge } from 'tailwind-merge'; export const cn = (...a) => twMerge(clsx(a))`),
...
const sCell = (v, best) => (v <= best ? `<td class="win">${kb(v)}</td>` : v === sizes.cn.gz || v === sizes.cn.min ? `<td class="lose">${kb(v)}</td>` : `<td>${kb(v)}</td>`)
const sizeRows = [
...
const libsJs = new TextDecoder().decode(libs.outputFiles[0].contents).replace(/<\/script/g, '<\\/script')
