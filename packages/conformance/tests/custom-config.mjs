...
import { extendTailwindMerge, validators as twmValidators } from 'tailwind-merge'
import { createCn, createTwMerge, validators, defaultConfig, mergeConfigs } from 'cn/config'
...
{
    const ext = { extend: { classGroups: { 'font-size': [{ text: ['hero', 'tiny'] }] } } }
    compare('extend-literals', createCn(ext), extendTailwindMerge(ext), [
        'text-hero text-lg', 'text-lg text-hero', 'text-tiny text-hero',
        'text-hero text-red-500', 'text-red-500 text-hero leading-6',
        'text-hero/7 text-lg', 'hover:text-hero hover:text-sm', 'text-hero text-hero',
    ])
}
...
// 2. extend a theme scale
{
    ...
    compare('extend-theme', createCn(ext), extendTailwindMerge(ext), [
        'text-huge text-lg', 'text-lg text-huge', 'text-huge text-[#333]',
        'md:text-huge md:text-sm', 'text-huge/8 leading-4',
    ])
}
...
// 3. custom validator function (same fn both sides)
{
    ...
    compare('custom-validator-fn', createCn(ext), extendTailwindMerge(ext), [
        'text-12vw text-lg', 'text-lg text-12vw', 'text-12vw text-14vw',
        'text-12vw text-red-500', 'text-vw text-lg',
    ])
}

// 4. marker validators vs tailwind-merge validator functions
{
    const ours = createCn({ extend: { classGroups: { shadow: [{ shadow: [validators.isNumber] }] } } })
    ...
    compare('marker-validators', ours, theirs, [
        'shadow-99 shadow-lg', 'shadow-lg shadow-99', 'shadow-99 shadow-3.5',
        'shadow-99 shadow-red-500', 'shadow-x shadow-99',
    ])
}
...
// 5. new groups + conflictingClassGroups extension
{
    ...
    compare('extend-conflicts', createCn(ext), extendTailwindMerge(ext), [
        'bar-x foo-a', 'foo-a bar-x', 'foo-a foo-b', 'bar-x bar-y foo-b',
        'hover:bar-x hover:foo-a', 'foo-a p-2 bar-y',
    ])
}
...
// 6. override semantics
{
    ...
    compare('override', createCn(ext), extendTailwindMerge(ext), [
        'shadow-custom shadow-lg', 'shadow-lg shadow-custom', 'shadow-custom shadow-custom',
        'shadow-sm shadow', 'shadow-custom text-lg',
    ])
}
...
// 7. prefix
{
    ...
    compare('prefix', ours, theirs, [
        'tw:p-2 tw:p-4', 'p-2 tw:p-4', 'tw:p-2 p-4', 'tw:hover:p-2 tw:hover:p-4',
        'tw:p-2! tw:!p-4', 'tw:hover:md:p-2 tw:md:hover:p-4', 'tw: tw:p-2',
        'tww:p-2 tw:p-4', 'tw:foo tw:bar', 'tw:-mt-2 tw:mt-4', 'tw:text-lg/7 tw:text-xl',
...
// 8. function-form config (identity transform must equal default behavior)
{
    ...
    compare('function-form', ours, theirs, [
        'p-2 px-4 p-6', 'text-lg/7 leading-6', 'hover:md:p-2 md:hover:p-4',
    ])
}

// 9. createTwMerge variadic shape
{
    const ours = createTwMerge({ extend: { classGroups: { 'font-size': [{ text: ['hero'] }] } } })
    const theirs = extendTailwindMerge({ extend: { classGroups: { 'font-size': [{ text: ['hero'] }] } } })
    const cases = [
        [['text-hero', 'text-lg']],
    ...
    for (const [argsArr] of cases) {
        diff('createTwMerge', JSON.stringify(argsArr), theirs(...argsArr), ours(...argsArr))
    }
}
...
// 10. mini-fuzz over an extended config
{
    const ext = {
        extend: {
            classGroups: { 'font-size': [{ text: ['hero', 'tiny'] }], foo: ['foo-a', 'foo-b'] },
            conflictingClassGroups: { foo: ['bar'] },
            theme: { text: ['huge'] },
        },
    }
    ...
    const pool = [
        'text-hero', 'text-tiny', 'text-huge', 'text-lg', 'text-sm', 'text-[22px]',
        'foo-a', 'foo-b', 'p-2', 'px-4', 'hover:text-hero', 'md:text-huge',
        'text-hero/7', 'leading-6', '!text-hero', 'text-hero!', 'unknown-x',
    ]
    let seed = 0xabcdef
    const rnd = () => {
        seed ^= seed << 13; seed >>>= 0
    ...
    for (let i = 0; i < 5000; i++) {
        ...
        const parts = []
        for (let k = 0; k < len; k++) parts.push(pool[Math.floor(rnd() * pool.length)])
        const s = parts.join(' ')
...
// 11. defaultConfig() / mergeConfigs exports behave sanely
{
    ...
    if (!cfg.classGroups.display) {
        fail++
        report.push({ label: 'defaultConfig', input: '-', expected: 'display group', actual: 'missing' })
    } else pass++
    const merged = mergeConfigs(defaultConfig(), { extend: { classGroups: { display: ['zzz'] } } })
    const ours = createCn(merged)
    diff('mergeConfigs', 'zzz block', extendTailwindMerge({ extend: { classGroups: { display: ['zzz'] } } })('zzz block'), ours('zzz block'))
}
